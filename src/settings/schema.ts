...
export type DirectDownloadMode = typeof DIRECT_DOWNLOAD_MODE_OPTIONS[keyof typeof DIRECT_DOWNLOAD_MODE_OPTIONS];
...
export type UserSettings = Omit<typeof DEFAULT_USER_SETTINGS, 'DIRECT_DOWNLOAD_MODE'> & {
    DIRECT_DOWNLOAD_MODE: DirectDownloadMode;
};

export function resolveDirectDownloadMode(value: unknown, legacyVisible: unknown, legacyAll: unknown): DirectDownloadMode {
    if (value === DIRECT_DOWNLOAD_MODE_OPTIONS.ASK ||
        value === DIRECT_DOWNLOAD_MODE_OPTIONS.VISIBLE ||
        value === DIRECT_DOWNLOAD_MODE_OPTIONS.ALL) return value;

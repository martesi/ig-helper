...
export function translateText() {
    const eLocale: Record<string, Record<string, string | string[]>> = {
        "en-US": {
    ...
    const allLocales = { ...eLocale, ...state.locale };
    return Object.fromEntries(Object.entries(allLocales).sort(([a], [b]) => a.localeCompare(b)).map(([lang, entries]) => [
        lang,
        Object.fromEntries(Object.entries(entries).map(([key, value]) => [key, Array.isArray(value) ? value.join('\n') : value])),
 ...
 * @return {void}
 */
export function _i18n(text: string): string {
    ...
    if (translate[state.lang] != undefined && translate[state.lang][text] != undefined) {
        ...
        return translate["en-US"][text] ?? text;
    }
}
...
export function repaintingTranslations() {
    document.querySelectorAll<HTMLElement>('[data-ih-locale]').forEach(element => {
        element.textContent = _i18n(element.dataset.ihLocale ?? '');
    });
    document.querySelectorAll<HTMLElement>('[data-ih-locale-title]').forEach(element => {

...
function getHighlightsStoryUsername() {
    const href = $('body > div section:visible a[href^="/"]').filter(function () {
...
function mountHighlightControlBar($element: JQuery<Element>, username: string, mediaType: 'video' | 'image') {
    const parent = $element?.[0];
    if (!(parent instanceof HTMLElement)) return null;
    parent.style.position = 'relative';

    let host = Array.from(parent.children).find((element): element is HTMLElement => element instanceof HTMLElement && element.classList.contains('IG_HIGHLIGHT_CONTROL_BAR'));
    if (!host) {

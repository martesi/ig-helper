...
import { ControlBar, IconButton } from '../../shared/ui/components.tsx';
import { RotateCcwIcon, RotateCwIcon, XIcon } from '../../shared/ui/icons.tsx';
...
export function openImageViewer(imageUrl: string) {
    removeImageViewer();
...
export function removeImageViewer() {
    ...
    if (!root) return;
    if (root.shadowRoot) render(null, root.shadowRoot);
    root.remove();
}

function ImageViewer({ imageUrl, returnFocus }: { imageUrl: string; returnFocus: Element | null }) {
    const rootRef = useRef<HTMLDivElement>(null);
    const sectionRef = useRef<HTMLElement>(null);
    const transformRef = useRef<HTMLDivElement>(null);
    const dragRef = useRef<{ moved: boolean; startX: number; startY: number } | null>(null);
    ...
    function zoomAt(event: WheelEvent | MouseEvent | KeyboardEvent, requestedScale?: number) {
        event.preventDefault();
        const rect = transformRef.current?.getBoundingClientRect();
        if (!rect) return;
        setTransform(current => {
            const scale = requestedScale ?? Math.min(5, Math.max(1,
                current.scale + (event instanceof WheelEvent && event.deltaY < 0 ? 0.1 : -0.1) * current.scale));
            const mouseX = ('clientX' in event ? event.clientX : rect.left + rect.width / 2) - rect.left;
            const mouseY = ('clientY' in event ? event.clientY : rect.top + rect.height / 2) - rect.top;
    ...
    function toggleZoom(event: MouseEvent | KeyboardEvent) {
        event.preventDefault();
